		...
		$.ajax({
            ...
            success:function(dt){
                   ...
                   str +="<p><span>结算开始时间：</span>"+checkNullStr(datas.beginTimeStr)+"</p>";
					...
					str +="<p><span>订单数量：</span>"+datas.orderQty+"</p>";
					str +="<p><span>应结款：</span>"+datas.needPay+"</p>";
					str +="<p><span>销售额：</span>"+datas.salesAmt+"</p>";
					if(datas.confirmStatus==1){
						str +="<p><span>商家确认状态：</span>待确认</p>";
					}else if(datas.orderState==2){
						str +="<p><span>商家确认状态：</span>不同意</p>";
					}else if(datas.orderState==3){
						str +="<p><span>商家确认状态：</span> 其他</p>";
					}
	...
	<jsp:include page="/view/include/leftMenu.jsp"  />
		...
		<div class="main">
			...
			<div class="good_state  mt_15">
				<input type="hidden" id="accoutNo" value="${tranMoneyDTO.accoutNo}">
				<input type="hidden" id="accountId" value="${tranMoneyDTO.id}">
				<ul>
					<li>结算状态：<span>
					<c:choose>
						<c:when test="${tranMoneyDTO.settleStatus==1}">   
							    未结算
						</c:when>
						<c:otherwise>
							    已结算
					...
					<li>应结款：<span>${tranMoneyDTO.needPay}</span></li>
